...
public class SubjectModel {
	...
	public Integer nextPk() throws SQLException {
		...
		try {
			...
			StringBuffer sql = new StringBuffer("select max(id) from subject");
	...
	public long add(SubjectBean bean) throws SQLException, DuplicateRecordException, RecordNotFoundException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			StringBuffer sql = new StringBuffer("insert into subject values (?,?,?,?,?,?,?,?,?,?)");
			pstm = conn.prepareStatement(sql.toString());
			pstm.setLong(1, pk);
	...
	public void delete(SubjectBean bean) throws SQLException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			StringBuffer sql = new StringBuffer("delete from subject where id=?");
			pstm = conn.prepareStatement(sql.toString());
	...
	public void update(SubjectBean bean) throws SQLException, DuplicateRecordException, RecordNotFoundException {
		...
		try {
			...
			StringBuffer sql = new StringBuffer(
					"update subject set course_name=?,course_id=?,subject_name=?,subject_id=?,description=?,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where id=?");
			pstm = conn.prepareStatement(sql.toString());
	...
	public SubjectBean findBySubjectName(String subjectName) throws SQLException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			StringBuffer sql = new StringBuffer("select * from subject where subject_name=?");
			pstm = conn.prepareStatement(sql.toString());
