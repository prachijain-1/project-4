...
public class MarksheetModel {
	...
	public long add(MarksheetBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			pstmt.setString(7, bean.getMaths());
...
//		}
		...
		try {
			...
			pstmt.setString(4, bean.getPhysics());
			pstmt.setString(5, bean.getChemistry());
			pstmt.setString(6, bean.getMaths());
	...
	public List search(MarksheetBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getPhysics() != null && bean.getPhysics().length() > 0) {
				sql.append(" AND PHYSICS = " + bean.getPhysics());
			}
			if (bean.getChemistry() != null && bean.getChemistry().length() > 0) {
				sql.append(" AND CHEMISTRY = " + bean.getChemistry());
			}
			if (bean.getMaths() != null && bean.getMaths().length() > 0) {
	...
	public List getMeritList(int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT*, (PHYSICS + CHEMISTRY + MATHS) as total,(((PHYSICS + CHEMISTRY + MATHS)/300)*100 ) as percentage from ST_MARKSHEET where PHYSICS>40 and CHEMISTRY>40 and MATHS>40 order by total desc");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
