...
public class UserModel {
	...
	public void update(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false); // Begin transaction
			String sql = "UPDATE ST_USER SET FIRST_NAME=?,LAST_NAME=?,LOGIN=?,PASSWORD=?,DOB=?,MOBILE_NO=?,ROLE_ID=?,UNSUCCESSFUL_LOGIN=?,GENDER=?,LAST_LOGIN=?,REGISTERED_IP=?,LAST_LOGIN_IP=?,CREATED_BY=?,MODIFIED_BY=?,CREATED_DATETIME=?,MODIFIED_DATETIME=? WHERE ID=?";
	...
	public boolean resetPassword(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		// String newPassword = String.valueOf(new
		// Date().getTime()).substring(0, 4); //
